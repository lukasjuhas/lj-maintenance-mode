...
class ljMaintenanceMode {
	...
	function __construct() {
		...
		if ( $is_enabled || isset( $_GET['ljmm'] ) && $_GET['ljmm'] == 'preview' ) : add_action( 'get_header', array( $this, 'maintenance' ) );
		endif;
	...
	function maintenance() {
		if ( 'preview' == ! ( current_user_can( 'administrator' ) || current_user_can( 'super admin' ) ) || ( isset( $_GET['ljmm'] ) && $_GET['ljmm'] ) ) {
			$content = get_option( 'ljmm-content' );
			if ( empty( $content ) ) {
				// Fallback.
				$content = '<h1>Website Under Maintenance</h1><p>Our Website is currently undergoing scheduled maintenance. Please check back soon.</p>';
